...
const Page = (props) => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await axios.get(`https://task-manager-dev-town-server.vercel.app/tasks?userId=${props.userId}`);
  ...
  const addTask = async () => {
    if (newTask.title && newTask.description && newTask.dueDate) {
      try {
        ...
        const response = await axios.post("https://task-manager-dev-town-server.vercel.app/task", {
          user:props.userId,
          title: newTask.title,
  ...
  const updateTask = async () => {
    if (
      ...
      try {
        await axios.put(
          `https://task-manager-dev-town-server.vercel.app/update/${editTask._id}`,
          editTask
        );
        setTasks((prevTasks) =>
          prevTasks.map((task) =>
  ...
  const deleteTask = async (taskId) => {
    try {
      ...
      await axios.delete(`https://task-manager-dev-town-server.vercel.app/delete/${taskId}`);
